...
public class MainFrame extends JFrame {
	...
	public MainFrame() {
		...
		layout2.setHorizontalGroup(layout2.createSequentialGroup().addContainerGap()
				.addGroup(layout2.createParallelGroup(Alignment.TRAILING)
						.addGroup(layout2.createSequentialGroup().addComponent(labelFrom).addGap(SMALL_GAP)
								.addComponent(textFieldFrom).addGap(LARGE_GAP).addComponent(labelTo).addGap(SMALL_GAP)
								.addComponent(textFieldTo))
						.addComponent(scrollPaneOutgoing).addComponent(sendButton))
				.addContainerGap());
		layout2.setVerticalGroup(layout2.createSequentialGroup().addContainerGap()

				.addGroup(layout2.createParallelGroup(Alignment.BASELINE).addComponent(labelFrom)
						.addComponent(textFieldFrom).addComponent(labelTo).addComponent(textFieldTo))
				.addGap(MEDIUM_GAP).addComponent(scrollPaneOutgoing).addGap(MEDIUM_GAP).addComponent(sendButton)
				.addContainerGap());
		...
		layout1.setVerticalGroup(layout1.createSequentialGroup().addContainerGap().addComponent(scrollPaneIncoming)
				.addGap(MEDIUM_GAP).addComponent(messagePanel).addContainerGap());
	...
	private void sendMessage() {
		try {
			...
			String[]invoker=destinationAddress.split("\\.");
			if(invoker.length!=4)
			{
				JOptionPane.showMessageDialog(this, "Invalid syntex", "Ошибка",
						JOptionPane.ERROR_MESSAGE);
			...
			for(int i=0;i<invoker.length;i++) {
				int faceless=-1;
				try {
				faceless=Integer.parseInt(invoker[i]);
				}
				catch(Exception e) {
					JOptionPane.showMessageDialog(this, "It isn`t a number", "Ошибка",
							JOptionPane.ERROR_MESSAGE);
					textFieldTo.requestFocusInWindow();
					return;
				}
					
				if(faceless<0 && faceless>255) {
					JOptionPane.showMessageDialog(this, "It is not correct ip", "Ошибка",
							JOptionPane.ERROR_MESSAGE);
					textFieldTo.requestFocusInWindow();
					return;
				}
